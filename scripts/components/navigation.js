  ...
  if (navType === "main"){
    ...
    const mainItems = menuItems.filter(item => item.showOnMain);
      navItemsHTML = mainItems.map(item => {
        ...
        return `
            <li><a href="#<span class="math-inline">\{item\.path\}" class\="</span>{activeClass}" data-path="<span class="math-inline">\{item\.path\}"\></span>{item.label}</a></li>
        `;
    }).join('');
  } else {
    const bottomItems = menuItems.filter(item => item.showOnBottom);
      navItemsHTML = bottomItems.map(item => {
          ...
          return `
            <li><a href="#<span class="math-inline">\{item\.path\}" class\="</span>{activeClass}" data-path="<span class="math-inline">\{item\.path\}"\></span>{item.icon ? `<i class="${item.icon}"></i>` : ''}<span>${item.label}</span></a></li>
        `;
      }).join('');
  }
    return navItemsHTML;
